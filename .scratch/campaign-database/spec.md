...
- [x] Replace `History.jsx` with a **Timeline** view (`Timeline.jsx`) backed by `GET /api/events`: chronological by `gameDate` (newest first), filterable by type/region, `postedAt` shown as secondary metadata, warnings visible per entry. The "log a new entry" form covers `ResourceChanged`/`DMRuling` only (same scope as Q17's paste-and-parse), auto-detecting which based on whether resource changes were entered, and can optionally settle an obligation.
- [x] Obligation progress display: `ObligationsPanel` in `Timeline.jsx` — a meter per open obligation (amount remaining vs. total, due date), reusing the existing `.meter` gauge styling from the Dashboard.
- [x] Reworked `Dashboard.jsx`, `CalendarView.jsx`, `Settlements.jsx`, `Codex.jsx` so edits emit `POST /api/events` instead of blind whole-resource `PUT`s: Dashboard diffs its draft against the loaded snapshot into a `ResourceChanged`; CalendarView emits `CalendarAdvanced`; Settlements emits `BuildingConstructed`/`BuildingRemoved` (with the new `displayName` field surfaced); Codex's three tabs now have three genuinely different shapes rather than one copy-pasted one — Introduction is read-only reference data (Q7), Deities saves per-card as `DeityAmended`, Locations batches into one whole-document `LocationAmended` with a required note. A shared `useEventSubmit` hook (`client/src/lib/`) and `WarningsList` component replace the load/edit/save/warnings boilerplate that used to be duplicated per view — this is the "collapse Codex.jsx's triplicate" outcome the earlier architecture review called out, generalized to every view rather than just the one file.
- [x] **Cutover**: the old JSON-file-backed routes (`GET`/`PUT /api/:resource`, `POST`/`DELETE /api/history`) are removed from `server/index.js` entirely — the database is now what the running app actually reads and writes. `data/*.json` stays in place as the git-diffable backup `npm run export` produces, not something the live app touches.
- [x] Fixed two gaps `read.js`'s calendar projection was missing (`monthName`, needed by `lib/campaign.js`'s `formatDate`; and `era`/`daysPerMonth`, read directly by `CalendarView.jsx`) — neither was caught until actually wiring the client to the projection endpoint.

**Not yet verified in a real browser** — no browser-automation tool was available in this session. Verified instead: `npm run build --prefix client` succeeds, `npm run lint --prefix client` is clean, `npm test` passes (25 tests), and every event shape each component sends was independently exercised against the live API via curl (ResourceChanged with an obligation reference, BuildingConstructed with `displayName`, LocationAmended's note requirement). Load the app and click through before trusting this fully.
